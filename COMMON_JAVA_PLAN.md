...
`common-java` uses Java 26, Spring Boot 4, and the published `com.gym.proto:gym-proto-java:7.0.2` contract. The `develop` version is `3.0.0-SNAPSHOT`; no coordinated G10 package or RC has been published.
...
The reusable Kafka contract workflow provisions Kafka and Schema Registry 7.7.1, validates all eleven immutable `gym-proto v7.0.2` fixtures, verifies Registry non-mutation, and uploads sanitized reports.
